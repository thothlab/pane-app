...
//!
//! Bareword without `:` is treated as substring search across url_path.
//! Status/size/duration values may use `N..M` range form.
...
pub fn compile_to_sql(input: &str) -> Result<(String, Vec<Box<dyn ToSql>>)> {
    ...
    for raw in tokenize(input) {
        ...
        let frag = match key {
            ...
            "__bare" => like_clause("url_path", value, negate, &mut params),
