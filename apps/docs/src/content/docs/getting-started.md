...
5. На Android без root — выполни manual install по инструкции в Devices.
   Блок «How to install the CA certificate» по умолчанию свёрнут (чтобы не
   занимать место и не рассеивать внимание); разверни его кликом по
   заголовку — внутри пошаговая инструкция и путь к `pane-ca.pem`.
...
Главное окно Pane запоминает свой размер и положение между запусками —
растянул/подвинул один раз, и при следующем старте окно откроется там же,
а не по центру с дефолтным размером. Первый запуск (когда сохранять ещё
нечего) — по центру. Отдельные окна Logcat это не затрагивает: они всегда
открываются своим фиксированным размером.
...
- **Pause** (Space) — буфер замирает, поток продолжается на бэке. **Clear** (⌘K) — чистит буфер. **Tail** — авто-прокрутка к новым строкам (отключается автоматически если пользователь сам прокрутил вверх). Пока Tail выключен, входящие батчи копятся в стороне (таблица не дёргается под глазами), а в статус-баре появляется бейдж **+N new** — сколько накопилось строк, **проходящих активный фильтр** (ровно столько добавится в таблицу при включении Tail; если под фильтр ничего не попадает, бейджа нет — пустого «+N» от фонового firehose больше не будет). Включить Tail обратно (или просто прокрутить до самой нижней строки) → накопленное сливается в таблицу, авто-скролл возобновляется.
- **Панель FILTERS слева** — сохранённые фильтры. Звезда ⭐ в поле фильтра сохраняет текущий запрос; в панели клик применяет фильтр, наведение открывает переименование (карандаш, inline-правка) и удаление (✕). Ширину панели можно перетащить разделителем (двойной клик по нему — сброс).
...
- **Выделение строк и копирование** (как в LogRabbit): клик выделяет строку, Shift-клик — диапазон, ⌘-клик — добавить/убрать, зажал и протащил — выделить несколько. **⌘C** копирует выделенные строки целиком (threadtime: время · pid · tid · level · tag: message), **⇧⌘C** — только поле message. Правый клик → меню **Copy / Copy message / View message**; Copy-действия работают над всем выделением. (Свободное выделение произвольного текста — в текстовом режиме, см. ниже.)
- **Просмотр строки** — двойной клик по строке или **View message** из меню. Окно **ресайзится** (тяни правый-нижний угол) и скроллится. Для одной строки сверху список полей (Time/App/PID/TID/Level/Tag) с копированием каждого по отдельности; ниже — содержимое (message, либо message’ы выделенных строк). Если содержимое похоже на JSON — **подсветка JSON** и кнопка **Format**, которая форматирует даже неполный/битый JSON по структуре (как LogRabbit).
- **Текстовый режим** (кнопка **Text**) — таблица заменяется плоским текстом отфильтрованных строк (снимок на момент включения): нативное выделение любого фрагмента, горизонтальный скролл длинных строк, копирование — то, что виртуализованный грид не умеет.
...
- Поток **байт-безопасен**: строки читаются сырыми байтами и декодируются `from_utf8_lossy` (битые байты → `�`). Android-устройства регулярно сыплют не-UTF-8 (Latin-1/бинарь) в лог-сообщениях; раньше один такой байт ронял read-loop в бесконечный reconnect, который заново дампил весь буфер — таблица «перерисовывалась» при стоящих timestamps. Теперь один битый байт не рвёт стрим.
