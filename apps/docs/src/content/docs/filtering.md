...
| `device:` | точный id устройства-источника | `device:<id>` |
...
устройством-источником. В списке есть колонка **Device** (имя устройства),
а справа от строки поиска — выпадающий список: выбор устройства
оставляет только его запросы (подставляет `device:<id>` в фильтр),
«Все устройства» снимает фильтр.
