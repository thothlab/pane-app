...
4. **Devices → Add device**. Pane находит подключённые телефоны через
   `adb` / `libimobiledevice`. Каждая строка подписана как
   `<вендор> <модель> · Android <версия> · <хвост-серийника>` —
   два одинаковых телефона воткнутых одновременно всё равно
   различимы за счёт последних шести символов USB-серийника. Выбери
   телефон → **+ Add**. Дальше Pane
   автоматически:
...
- **Копировать** — кладёт в буфер обмена полный дамп этого запроса
  (стартовая строка → заголовки → тело → пустая строка → ответ → его
  заголовки → его тело) в формате OkHttp `HttpLoggingInterceptor`. Тот
  же синтаксис, что у Android-логгера, поэтому дамп вставляется в баг-
  репорт рядом с собственными логами приложения без переформатирования.
  Тело декодируется как UTF-8; бинарное содержимое заменяется маркером
  `[N-byte binary body]` чтобы не засорять буфер. Лимит — 8 MiB на тело,
  при truncation добавляется отдельная пометка.
- **Добавить в правила** — пресоздаёт stub-правило из текущего capture
  (см. [Подмена ответов](/docs/rules/) → «Быстрый способ: из Captures»).

Кнопка **Tail** на тулбаре прибивает список к последней записи.
Если её выключить (или прокрутить вверх) — список замирает: периодические
обновления приостанавливаются, пока ты не нажмёшь Tail обратно или не
...
- **Pause** (Space) — буфер замирает, поток продолжается на бэке. **Clear** (⌘K) — чистит буфер. **Tail** — авто-прокрутка к новым строкам (отключается автоматически если пользователь сам прокрутил вверх). Пока Tail выключен, входящие батчи копятся в стороне (таблица не дёргается под глазами), а в статус-баре появляется бейдж **+N new** — сколько накопилось. Включить Tail обратно (или просто прокрутить до самой нижней строки) → накопленное сливается в таблицу, авто-скролл возобновляется.
...
- **Regex через `~`** — pattern компилируется как нативный JS `RegExp` и матчится против `tag` ИЛИ `message`. Регистр учитывается (флаги `/.../i` не передать; workaround — character class `[Oo]`):
  ```text
  ~OkHttp                              # substring через regex (эквивалент bareword)
  ~^Choreographer                      # tag/message НАЧИНАЕТСЯ с Choreographer
  ~Connection (refused|reset|aborted)  # альтернативы через |
  ~\d+ms\b                             # число + "ms" — поиск меток задержки
  ~^(?!.*heartbeat).+                  # negative lookahead — всё, что НЕ heartbeat
  ~stack.*overflow                     # любые символы между словами
  !~^E/                                # внешнее ! — выкинуть всё matching
  ~slow tag:OkHttp level:W..F          # AND с другими ключами
  ```
- **Поиск в отфильтрованном** (`⌘⇧F`) — отдельное поле справа от фильтра: substring-поиск поверх уже отфильтрованных строк с подсветкой совпадений. Удобно когда фильтр сужает firehose до сотен/тысяч строк, а среди них надо быстро найти конкретное слово.
- **Status bar** внизу окна: `Showing N of M` (`M+` означает что буфер уперся в MAX_ENTRIES = 100k и старые записи уходят FIFO).
...
- **⌘F** — фокус в поле фильтра. **⌘⇧F** — фокус в поле поиска.
