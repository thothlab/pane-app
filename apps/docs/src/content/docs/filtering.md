...
устройством-источником. В списке есть колонка **Device** (короткое имя, напр. «CipherLab RS35»),
а справа от строки поиска — выпадающий список: выбор устройства
оставляет только его запросы (подставляет короткий `device:<имя>` в
фильтр), «Все устройства» снимает фильтр. Можно и вручную:
