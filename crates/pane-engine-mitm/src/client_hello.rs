...
pub(crate) async fn peek_fingerprint(stream: &TcpStream) -> ClientFingerprint {
    ...
    loop {
        let peeked = match tokio::time::timeout_at(deadline, stream.peek(&mut buf)).await {
            Ok(Ok(0)) => return ClientFingerprint::unknown(), // client hung up
            Ok(Ok(n)) => n,
            Ok(Err(_)) | Err(_) => return ClientFingerprint::unknown(),
        };
        match parse_ja3(&buf[..peeked]) {
            Ok(ja3) => return fingerprint_of(&ja3),
            ...
            Err(ParseError::NotAClientHello) => return ClientFingerprint::unknown(),
        }
    }
}
...
/// Build the JA3 string: `version,ciphers,extensions,groups,point_formats`.
pub(crate) fn parse_ja3(bytes: &[u8]) -> Result<String, ParseError> {
...
mod tests {
    ...
    fn builds_a_ja3_string() {
        ...
        assert_eq!(parse_ja3(&hello).unwrap(), "771,4865-4866,0-10-11,29-23,0");
    }
    ...
    fn two_stacks_fingerprint_differently() {
        ...
        assert_ne!(parse_ja3(&a).unwrap(), parse_ja3(&b).unwrap());
        assert_ne!(
            fingerprint_of(&parse_ja3(&a).unwrap()),
            fingerprint_of(&parse_ja3(&b).unwrap())
        );
    }
    ...
    fn the_same_stack_fingerprints_identically_across_connections() {
        ...
        assert_eq!(parse_ja3(&a).unwrap(), parse_ja3(&b).unwrap());
    }
    ...
    fn grease_is_ignored() {
        ...
        assert_eq!(parse_ja3(&plain).unwrap(), parse_ja3(&greased).unwrap());
    }
