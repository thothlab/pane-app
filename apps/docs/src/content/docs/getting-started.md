...
- **Pause** (Space) — буфер замирает, поток продолжается на бэке. **Clear** (⌘K) — чистит буфер. **Follow** — авто-прокрутка к новым строкам (отключается автоматически если пользователь сам прокрутил вверх). Пока Follow выключен, входящие батчи копятся в стороне (таблица не дёргается под глазами), а в статус-баре появляется бейдж **+N new** — сколько накопилось. Включить Follow обратно (или просто прокрутить до самой нижней строки) → накопленное сливается в таблицу, авто-скролл возобновляется.
