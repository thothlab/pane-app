...
- **Regex через `~`** — pattern компилируется как нативный JS `RegExp` и матчится против `tag` ИЛИ `message`. Регистр учитывается (флаги `/.../i` не передать; workaround — character class `[Oo]`):
  ...
  ```
- **Поиск** (`⌘F`) — отдельное поле справа от фильтра: substring-поиск, который **подсвечивает** совпадения во всех видимых колонках (время, приложение, тег, сообщение) и **не сужает** список строк (это задача фильтра). **Enter** — перейти к следующему совпадению (вниз), **Shift+Enter** — к предыдущему (вверх); текущая строка обводится рамкой, рядом счётчик `i/N`. Удобно быстро найти конкретное слово в том, что уже на экране.
...
- **⌘F** — фокус в поле поиска. **⌘⇧F** — фокус в поле фильтра.
- **Выделение строк и копирование** (как в LogRabbit): клик выделяет строку, Shift-клик — диапазон, ⌘-клик — добавить/убрать, зажал и протащил — выделить несколько. **⌘C** копирует выделенные строки целиком (threadtime: время · pid · tid · level · tag: message), **⇧⌘C** — только поле message. Правый клик → меню **Copy / Copy message / View message**; Copy-действия работают над всем выделением.
- **Просмотр строки** — двойной клик по строке или **View message** из меню. Окно **ресайзится** (тяни правый-нижний угол) и скроллится. Для одной строки сверху список полей (Time/App/PID/TID/Level/Tag) с копированием каждого по отдельности; ниже — содержимое (message, либо message’ы выделенных строк). Сверху — **поле поиска** (`⌘F` внутри окна): подсветка совпадений с навигацией Enter (вперёд) / Shift+Enter (назад) и счётчиком. Если содержимое похоже на JSON — **подсветка JSON** и кнопка **Format**, которая форматирует даже неполный/битый JSON по структуре (как LogRabbit).
