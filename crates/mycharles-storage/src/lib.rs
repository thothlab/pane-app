...
impl CaRecord {
    pub fn into_dto(self) -> mycharles_ipc::CaCertificateDto {
        mycharles_ipc::CaCertificateDto {
            id: self.id,
...
impl Storage {
    ...
    pub fn clear_captures(&self, _older_than: Option<String>) -> Result<usize> {
        ...
        // Body GC: simple — find orphaned bodies and delete.
        Ok(n)
    }
        ...
        match format {
            ...
            "har_single" => {
                let har = serde_json::json!({
                    "log": { "version": "1.2", "creator": {"name": "my-charles", "version": env!("CARGO_PKG_VERSION")},
                        "entries": [ {
