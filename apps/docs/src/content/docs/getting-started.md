...
- **Regex через `~`** — pattern компилируется как нативный JS `RegExp` и матчится против `tag` ИЛИ `message`. Регистр учитывается (флаги `/.../i` не передать; workaround — character class `[Oo]`):
  ...
  ```
- **Поиск** (`⌘F`) — отдельное поле справа от фильтра: substring-поиск, который **подсвечивает** совпадения во всех видимых колонках (время, приложение, тег, сообщение) и **не сужает** список строк (это задача фильтра). Удобно быстро найти конкретное слово в том, что уже на экране.
