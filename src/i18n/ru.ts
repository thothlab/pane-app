...
const ru: Dict = {
  ...
  settings: {
    ...
    poll_body:
      "Список приходит событиями: каждая завершённая запись приходит сама. Периодический опрос нужен только чтобы подстраховаться, если событие потерялось.",
    poll_enabled: "Периодически опрашивать",
    poll_interval: "Интервал",
    poll_seconds: "с",
    poll_hint:
      "По умолчанию выключено: список обновляется по событиям захвата, и этого достаточно. Включайте, если записи всё же теряются - тогда опрос подстрахует. Учтите, что во время автопрогона каждый опрос ложится нагрузкой на тот же бэкенд, которым пользуется прогон.",
