...
`device:RS35` в фильтр), «Все устройства» снимает фильтр. Можно и вручную:
`device:RS35` матчит по имени/серийнику (substring).

Атрибуция работает так: каждое USB-устройство ходит через свой
Mac-порт прокси, и backend определяет источник по этому порту. Старые
captures и устройства без атрибуции показываются как «—».
